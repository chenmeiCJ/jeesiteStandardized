...
<body>
	...
	<form:form id="inputForm" modelAttribute="dataElementProcess" action="${ctx}/dataelementprocess/dataElementProcess/saveAudit" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="form-actions">
			<shiro:hasPermission name="dataelementprocess:dataElementProcess:edit">
				<c:if test="${dataElementProcess.act.taskDefKey eq 'apply_end'}">
					<input id="btnSubmit" class="btn btn-primary" type="submit" value="审批" onclick="$('#flag').val('yes')"/>&nbsp;
				</c:if>
				<c:if test="${dataElementProcess.act.taskDefKey ne 'apply_end'}">
